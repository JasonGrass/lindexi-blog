...
pubDatetime: 2023-10-10 01:49:27
modDatetime: 2024-08-06 12:43:42
slug: WPF-笔迹算法-从点集转笔迹轮廓
description: "WPF 笔迹算法 从点集转笔迹轮廓"
tags:
  - WPF
---
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓0.png) -->
![](images/img-modify-604f205c6e689ef414bc3a7668483e78.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓1.png) -->
![](images/img-modify-bb17ce0a3141814974a3c7ec6158992d.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓2.png) -->
![](images/img-modify-2a9387915c651040981413db94320790.jpg)

接下来求每个点与下一个点相连的射线向量，再算出射线向量的法线方向，在此法线方向上以触摸点的中心向法线两端延伸线段，延伸的线段长度由笔迹粗细配置以及当前触摸点的压感系数决定，如下图，蓝色的线就是射线向量，黄色的线是射线向量的法线方向延伸的线段

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓3.png) -->
![](images/img-modify-c1e567859299d662f0cca6351594d379.jpg)

再获取线段的两个端点，如下图，红色的圆点就是延伸的线段的两个端点

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓4.png) -->
![](images/img-modify-f17177872a9015a1f95d2758e5a9d07b.jpg)

接着将各个线段的端点按照如下图的方式连接起来，各个线段的两个端点分别按照两边连接成两条折线，再将这两条折线和起始点和结束点连接到一起，构成闭合的 Path 几何形状，红色的折线就可以被当成笔迹轨迹的 Path 几何形状

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓5.png) -->
![](images/img-modify-2e89e82a6a8ab4818cc64886921fbe47.png)

最后将红色的折线组成的笔迹轨迹的 Path 几何形状填充，填充之后看起来的效果还行

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓6.png) -->
![](images/img-modify-059ccbea2cead9c254d9197c182cf23b.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓7.png) -->
![](images/img-modify-9e511114949fc082542885f4a3735775.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓8.png) -->
![](images/img-modify-156308db151e269a4e2b9724162facfd.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓9.png) -->
![](images/img-modify-ea9ee80b0e16780d35eab902cc3c469c.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓10.png) -->
![](images/img-modify-1f1f4529bff9303c6ea0e5b385bb21fa.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓11.png) -->
![](images/img-modify-413babfed91d921b86bd7b8c7a27c75e.jpg)

这时需要把这些点分为两个线段，分为两个线段的大概效果如下图

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓12.png) -->
![](images/img-modify-6257ea239c6c9e1eddc37626014d3b80.jpg)

对于汉字而言，我认为如果以上两个线段构成的内角在 90 度以下时，有棱的好看，超过 90 度时，使用圆角的好看

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓13.png) -->
![](images/img-modify-4a74ef554ef63f53b6ce5a42384f838d.jpg)

通过输入可以拿到触摸点，按照两个触摸点连接为线，求相邻线段的夹角，判断角度可以知道用户是否希望画出棱还是画出圆。加上这个优化之后就可以在写汉字时，比微软默认的 WPF 或 UWP 的笔迹算法在棱角方面处理更好

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓14.png) -->
![](images/img-modify-10c8ee806bd80a595e9ee9ec6ce75ea3.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓15.png) -->
![](images/img-modify-64881f552daea6e2fd63f18e5ea16c3f.jpg)

简单的笔迹轨迹宽度优化算法大概如下，下面将会用到一点点公式，相信大家一看就明白，以下使用到的公式

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓16.png) -->
![](images/img-modify-5a3765180d82dd80df6c25275969a08f.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓30.png) -->
![](images/img-modify-31c6ed33b2da61ac5125ebbe6393e7d5.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓17.png) -->
![](images/img-modify-240ff1a55e8a617851971aacfadcdafb.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓18.png) -->
![](images/img-modify-9aa3a9c1a60d1a20e4bacb49504a4735.jpg)

根据触摸点的每个点的状态可以决定骨架点的每个点的状态，对应的就是每个点的上下左右边距，如下图。决定每个点的上下左右边距算法叫做惯性边距算法，这个惯性边距算法将放在下文再描述

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓19.png) -->
![](images/img-modify-5a4f37dedcc8dcfbefe5dc874a4f371d.jpg)

经过了惯性边距算法，可以获取骨架点的上下左右边距，取边距的端点，作为笔廓点。如下图，笔廓点就是蓝色的圆圈

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓20.png) -->
![](images/img-modify-433697207522fbdc6022877692f1bdaf.jpg)

如下图，连接笔迹的笔廓点就可以获得笔迹的轮廓线，也就是获得笔迹的几何图形。但仅仅采用如上述算法，可以看到笔迹的轮廓相对粗糙，虽然比上文给的算法好了一点，但也没好多少。想要实现更好的效果，还需要继续添加更多逻辑

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓21.png) -->
![](images/img-modify-f76c924bdb5be82b09aa5f3ffbf244f1.jpg)

在开始介绍算法之前，需要引入不对称椭圆的概念，默认的椭圆都是对称的，如上下对称或左右对称。而不对称椭圆是上下左右都不对称的椭圆。如下图，从不对称椭圆的圆心的上下左右四个方向有着不同的长度

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓22.png) -->
![](images/img-modify-9b0c2fadb62acf4db64da533fa317b97.jpg)

不对称椭圆的算法相当于绘制出四个对称的椭圆，分别取其中的四分之一拼接起来的椭圆

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓23.png) -->
![](images/img-modify-55342a6a81252306931445361b8e455d.jpg)

如下图，是将绘制出来的四个对称的椭圆各取四分之一部分拼接起来，其中填充部分就是非对称椭圆

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓24.png) -->
![](images/img-modify-6b96a799058be88ab54f65e0ae0189eb.jpg)

这里的非对称椭圆是用在将笔迹的骨架点按照惯性边距算法上下左右分别采用不同的长度，创建出来的椭圆

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓25.png) -->
![](images/img-modify-82f602eb5314bd17a6abc389554b294f.jpg)

沿着椭圆的切线方向连接的线段就可以作出平滑的笔迹轮廓线，如下图。下图绘制仅仅只是参考，部分线段连接不是采用椭圆的切线

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓26.png) -->
![](images/img-modify-ae9613813c6f6ada6b9041f204cfa7a5.jpg)

特别的，为了性能优化部分，因为笔迹的粗细一般都很小，在笔迹粗细很小的时候，可以使用多边形近似代替椭圆。因为对多边形的求值计算的性能要远远高于椭圆，同时求椭圆切线的代码也不好写。如下图，采用如 米 字的方式代替椭圆

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓27.png) -->
![](images/img-modify-83cfe28dfa29d6993d38fb49cb059fdc.jpg)

只需要连接椭圆的外接轮廓点即可作出笔迹效果，如下图

<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓28.png) -->
![](images/img-modify-6d6a3e4eb9e12669ee19f7e7f1986b3b.jpg)
...
<!-- ![](images/img-WPF 笔迹算法 从点集转笔迹轮廓29.png) -->
![](images/img-modify-0ef6ae86c55f02b031b7d58d6c7cf709.jpg)
