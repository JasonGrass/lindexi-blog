...
<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期0.png) -->
![](images/img-modify-304343c12d0a4f2e7006a9b0affe3cee.jpg)
...
<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期1.png) -->
![](images/img-modify-8d2065a9a30ca99d75c894a87e2e3d53.jpg)
...
<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期2.png) -->
![](images/img-modify-5ea311eaeca1478f5dd3aeb0c9a54021.jpg)

继续按下 F5 向下跑，将会发现在 `F1_FxxEvent` 方法里面抛出异常，因为 F2 的 F3 属性为空

<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期3.png) -->
![](images/img-modify-620cc0ff861ed11f99f871fd6d88a046.jpg)
...
<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期4.png) -->
![](images/img-modify-455e9c8cdfd3127c919714ab725c32ce.jpg)

可见此时 `_f2` 字段还是 null 值，非常符合预期

继续进入 F2 构造函数，展开局部变量，尝试去看一下 f1 参数此时的 F2 属性，可以看到此时的属性已经有值了，且 `_f2` 字段也有值了

<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期5.png) -->
![](images/img-modify-81480cc8621119b4006d02a07b0651da.jpg)

但是回到代码里面，逻辑上应该是当前的 `_f2` 字段是没有值的，因为现在正在 F2 的构造函数里，只有 F2 构造函数执行结束之后，才会给 `_f2` 字段赋值

<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期6.png) -->
![](images/img-modify-84f2222b6391891a214d801cd899139e.jpg)
...
<!-- ![](images/img-警惕 Visual Studio 属性求值副作用导致逻辑不符合预期7.png) -->
![](images/img-modify-d1966c820b836256c9456f2b3072f633.jpg)
