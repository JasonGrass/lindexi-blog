...
pubDatetime: 2019-05-24 07:54:36
modDatetime: 2024-08-06 12:43:42
slug: WPF-源代码-从零开始写一个-UI-框架
description: "WPF 源代码 从零开始写一个 UI 框架"
tags:
  - WPF
  - 渲染
  - WPF源代码
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架1.png) -->

![](images/img-modify-3dc6a5e7032829d6e468e4d075cc5e87.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架2.png) -->

![](images/img-modify-9b962b3be8ccb46cb88786a5bacbbe6f.png)

刚才说到了绘制原语，需要解释一下，开始之前，先问一下，不知有没小伙伴不知道拼音的？如果学会了拼音，就可以使用拼音拼出普通话。对应的，电脑知道绘制原语就画出界面。从计算机图形学上，只要支持绘制原语就基本能画出所有类型的界面。

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架3.png) -->

![](images/img-modify-a0d08579eb6873d2d9d1dd804190f4e8.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架0.png) -->

![](images/img-modify-8f97aa040f01136304b82028a4a417e6.png)
...
现在一个最简单的 UI 框架已经完成，虽然还没有写输入的代码，但可以开始验证这个 UI 框架是否可行，可以使用真实的代码跑一下。
...
第三步尝试在画板画出元素。先创建一个 Board 出来，这就是画布。然后给画布注入渲染平台，也就是 `board.PlatformVisual = new DrawVisual()` 然后添加进入元素，接着调用 InvalidateVisual 方法触发绘制。
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架4.png) -->

![](images/img-modify-b0f80e8700d2bade38c0cb908a8a677f.png)

这里定义简单元素和组合元素，简单元素是由多个基础元素组成

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架5.png) -->

![](images/img-modify-23f088f242a460d02fa80f0363646a62.png)

多个简单的元素可以作为一个复杂元素，复杂元素实际就是 WPF 的按钮等元素

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架6.png) -->

![](images/img-modify-48ca9ab8e7102d74f57a87e7267ac367.png)

从定义可以看到，如果是一个简单元素，基础元素之间如何确定坐标？难道需要知道基础元素构成的简单元素所在画布的坐标，然后再计算基础元素相对于简单元素的内部坐标画在画布上？

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架7.png) -->

![](images/img-modify-d8c3e009341ccae3733be43e7043a13b.png)

框架就是用来做这部分的封装，这时需要引入容器的概念。

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架8.png) -->

![](images/img-modify-5223c99c7c373773951e3e4e95643019.png)

容器本身在画布上是有 Bounds 的概念，也就是容器相对于画布的坐标和容器本身的宽度和高度，同时容器提供容器内的坐标。

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架9.png) -->

![](images/img-modify-cea29e5a85c8480e94230c02592e38d4.png)

如果在容器内部放一个元素，元素只需要知道容器，不需要知道容器之外。元素知道元素在容器内部的坐标就可以，容器知道容器在画布的坐标，于是画出元素就只需要将元素的坐标加上容器的坐标

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架10.png) -->

![](images/img-modify-c8ab57d28fdf9ed4edb5b7cf6fa9e837.png)

现在可以看到画布有坐标、宽高的概念，容器有坐标、宽高的概念，元素也有坐标和宽高的概念，有了这些就可以开始做布局

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架11.png) -->

![](images/img-modify-38569918eec5705f2a7924da0ae3585f.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架12.png) -->

![](images/img-modify-bfc62470a33142a62ee7d51a7f8eed05.png)

容器和复杂元素都可以抽象为元素，容器里面可以放元素，元素里面可以通过容器再放元素。就和WPF的控件一样，在 WPF 的用户控件是可以放 Grid 的，这是一个面板控件，里面还可以继续套普通的元素或者再套一个 Grid 控件

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架13.png) -->

![](images/img-modify-d469c1a2f0bda573a4a1c75e49605f65.png)

这里是将容器看做特殊的元素，因为容器本身可以放元素，如果放的元素是容器，那么容器内部就可以嵌套容器。

于是一个复杂的元素，实际上就是容器和简单元素的组合

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架14.png) -->

![](images/img-modify-b9cdec36268769f06ae2c9a1a6e0df5c.png)

现在容器的概念已经清楚了，布局做的就是解决容器内部的元素如何排列的问题

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架15.png) -->

![](images/img-modify-72148d1600a2e9d97342c3d72193c832.png)

在 WPF 中有很多布局的控件，布局的控件如 Grid 等这些，实际上就是按照一定的规则排列元素

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架16.png) -->

![](images/img-modify-053da115bf521907b9a61892f23d2b23.png)

但是如 StackPanel 的控件，在排列元素布局之前，是需要知道元素的宽高和大小的

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架17.png) -->

![](images/img-modify-e4827e1c44a22ee8db7a2abcdc61dc55.png)

于是容器在布局之前是需要先做测量，测量就是获得容器里面的元素的宽度和高度。但是容器里面的元素假如是容器，就需要递归询问元素

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架18.png) -->

![](images/img-modify-a9caab078c678dde8de5ddbcff5c9222.png)

在知道了每个元素的宽高，如何布局就是业务的事情，这里就不是框架内部需要做的

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架19.png) -->

![](images/img-modify-b2897dc4b6c9cd9458554aae616652e3.png)

布局完成了，渲染也就是十分简单了，如有一个基础的元素，需要在画布渲染，只需要将这个元素外层所有容器的坐标和元素自己相对于容器的坐标加起来就可以计算出元素在画布的坐标，画出元素就可以了。

这样将一层层容器和元素组成的树就成为视觉树

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架20.png) -->

![](images/img-modify-9bf775b9898abc90d98e0248b99f1e14.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架21.png) -->

![](images/img-modify-646c09a56cf29ffdcfca7570d13baaeb.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架22.png) -->

![](images/img-modify-3caf77b794b7882617e7d7fbb0137320.png)

于是画布会找到画布里面的所有元素，对每个元素都创建一个 DrawingContext 给他，但是绘制原语部分都是引用相同的平台绘制

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架23.png) -->

![](images/img-modify-b57b77777759fac9708489ca9003dc97.png)

对于容器里面的元素，就需要递归给每个元素创建一个新的 DrawingContext 叠加上容器本身的外层坐标加上元素在容器的偏移的坐标

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架24.png) -->

![](images/img-modify-35efbcebe98a02eb8d000f85cdca7a1f.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架25.png) -->

![](images/img-modify-c1fd2ea9d01f0be7a1fb1e4d3c30d2bd.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架26.png) -->

![](images/img-modify-56b6bf9d3f2c8bafc1b1c34377ac72cf.png)

此时对于基础元素只需要关注元素内部的坐标进行绘制，如绘制一个三角形，就需要知道三角形是在元素的哪里进行绘制，而不需要关注这个元素是被放在哪里

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架27.png) -->

![](images/img-modify-b1e0c34a77043f40c1fd6ad78cc4b28b.png)

但是元素调用的绘制方法，如上面的代码实际是掉基础渲染的方法，在基础渲染的方法修改坐标加上外层坐标

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架28.png) -->

![](images/img-modify-d48bcdfc70cd97952d5265fdf5b9fba9.png)

框架的工作就是将画布视为容器，将容器视为元素，递归调用元素渲染，同时在调用的过程不断根据元素封装 DrawingContext 的坐标，当然对于基础元素就直接调用绘制的方法

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架29.png) -->

![](images/img-modify-58a980878ccd8ffac52a6f43ecfde09f.png)

这样就可以将元素投影到画布上，在渲染的时候是没有容器的概念，也没有复杂元素的概念，只有基础的元素的概念

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架30.png) -->

![](images/img-modify-006fb28241c6f8022967c06dacd14d17.png)

等等，是不是忘了什么，元素的层级怎么办？

元素的层级需要在控件布局的时候确定，实际上元素的层级只是调用元素渲染的顺序而已，按照调用顺序的不同从而确定了元素的层级。

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架31.png) -->

![](images/img-modify-5447a6116832830e72522fde12bfc17e.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架32.png) -->

![](images/img-modify-a16d2596739889f0136b67ac7dd9597e.png)

此时需要按照元素的层级排序，因为顶层的元素会挡住底层的元素，需要先计算顶层元素的命中测试才能计算底层的元素。当然在层级排序的时候需要去掉不做交互的元素。

<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架33.png) -->

![](images/img-modify-0c6b91b6410d7ff83cfab86f054a17e8.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架34.png) -->

![](images/img-modify-6bda269fe529ca13c4a3e4445f24723e.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架35.png) -->

![](images/img-modify-f6def325153b20d08406894a5ff6dc09.png)
...
<!-- ![](images/img-WPF 源代码 从零开始写一个 UI 框架36.png) -->

![](images/img-modify-357460642fe886d6b575b5b654d1e830.png)
